...
function Game(opts) {
  ...
  this.items = []
  this.detached = new Detached(this)
...
Game.prototype.createBlock = function(pos, val) {
  ...
  if (intersections[0]) {
    ...
    var ref = self.detached.refs[intersections[0].object.id]
    if (!ref) console.log('!ref ' + intersections[0].object.id);
    if (!ref) return
    ...
    var getInverse = function (m) {
      return new THREE.Matrix4().getInverse(m);
    };
    
    var tr = getInverse(ref.rotationObject.matrix).multiplyVector3(
      getInverse(ref.translationObject.matrix).multiplyVector3(pt)
    )
    var ci = self.voxels.chunkAtPosition(tr)
    var vv = self.voxels.voxelVector(tr)
    var vi = self.voxels.voxelIndex(vv)
    
    console.log(ci, vv.x, vv.y, vv.z)
    ref.set(ci, vi, 3)
    return true
  }
